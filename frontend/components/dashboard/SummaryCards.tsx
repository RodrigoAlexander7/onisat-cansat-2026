...
export function SummaryCards() {
  ...
  const elapsedFormat = currentPacket?.timestamp_ms
    ? (() => {
        const t = new Date(currentPacket.timestamp_ms);
  ...
  const press = currentPacket?.pres_ms5611?.toFixed(1) ?? '--';
  const temp = currentPacket?.temp_bme280?.toFixed(1) ?? '--';
  ...
  const power = currentPacket?.power_ina226?.toFixed(2) ?? '--';
    ...
    <div className="w-full overflow-x-auto">
      <div className="flex min-w-max items-center gap-2">
        <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white/80 p-1.5">
          ...
          <MetricCard title="Pressure" value={press} unit="Pa" />
        ...
        <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white/80 p-1.5">
          ...
          <MetricCard title="Power" value={power} unit="W" />
