...
export class CanSatStateMachine {
  ...
  public evaluate(packet: ProcessedTelemetry): MissionState {
    const currentAlt = packet.alt_ms5611 ?? 0; // Use MS5611 altitude
    const velZ = packet.velocity_z ?? 0;
    ...
    switch (this.currentState) {
      case MissionState.PRE_LAUNCH:
        ...
        if (altDiff > (TELEMETRY_CONSTANTS.LIFT_MIN_ALTITUDE_CM / 100)) {
          if (!this.liftConditionStartTime) {
            this.liftConditionStartTime = packet.timestamp_ms;
          } else if (packet.timestamp_ms - this.liftConditionStartTime >= TELEMETRY_CONSTANTS.LIFT_MIN_DURATION_MS) {
      ...
      case MissionState.LIFT_ASCEND:
        ...
        if (velZ < 0) {
           if (!this.dropConditionStartTime) {
             this.dropConditionStartTime = packet.timestamp_ms;
           } else if (packet.timestamp_ms - this.dropConditionStartTime >= 1500) {
        ...
        if (
          ...
          Math.abs(velZ) < 1.0 // near zero velocity
        ) {
      ...
      case MissionState.APOGEE:
        ...
        if (velZ < -2.0) { // Distinct downward velocity
      ...
      case MissionState.AUTOGYRO_DEPLOYED:
        ...
        if (Math.abs(altDiff) <= TELEMETRY_CONSTANTS.LANDED_ALTITUDE_TOLERANCE_M && Math.abs(velZ) < 0.5) {
