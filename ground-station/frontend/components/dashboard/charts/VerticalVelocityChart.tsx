...
const VELOCITY_Z_EXTRACTORS: DataExtractor[] = [(p) => p.velocity_z ?? null];
...
export function VerticalVelocityChart() {
  ...
  const options = useMemo<Omit<uPlot.Options, 'width' | 'height'>>(() => ({
    ...
    axes: [
      ...
      { label: 'Vertical Velocity', side: 1, values: (_u, vals) => vals.map((v) => `${v.toFixed(1)}m/s`) }
    ],
    scales: {
      x: { time: false },
      y: { auto: true }
  ...
  return (
    <UPlotWrapper 
      ...
      sensors={[
        { legend: 'Vel Z', color: '#10b981', unit: 'm/s', sensor: 'MS5611 (Calculated)' }
      ]}
    />
  );
}
